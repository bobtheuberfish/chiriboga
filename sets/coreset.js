...
var coreSet = [];
coreSet[1] = { title:'Noise: Hacker Extraordinaire', imageFile:'01001.png', player:runner, link:0, cardType:"identity",subTypes:["G-mod"],
	cardInstalled: {
		Resolve: function(card){
			if (CheckCardType(card,["program"]))
			{
				if (CheckSubType(card,"Virus")) Trash(corp.RnD.cards[corp.RnD.cards.length-1],true);
			}
		}
	}
};
coreSet[2] = { title:'Déjà Vu', imageFile:'01002.png', player:runner, cardType:'event', playCost:2,
	Enumerate: function(params) {
		return ChoicesArrayCards(runner.heap);
	},
	Resolve: function(params) {
		MoveCard(params.card,runner.grip);
		var choices = ChoicesArrayCards(runner.heap,function(card){return CheckSubType(card,"Virus");});
...
coreSet[3] = { title:'Demolition Run', imageFile:'01003.png', player:runner, cardType:'event', playCost:2, subTypes:["Run","Sabotage"],
		Enumerate: function() {
			var ret = [];
		...
		Resolve: function(params) {
			...
			phases.runAccessingCard.Resolve.trash = function() {
				...
				Trash(accessingCard,true,function(){ ResolveAccess(originalLocation); });
			};
...
coreSet[4] = { title:'Stimhack', imageFile:'01004.png', player:runner, cardType:'event', subTypes:["Run"], playCost:0,
	Enumerate: function() {
...
coreSet[5] = { title:'Cyberfeeder', imageFile:'01005.png', player:runner, cardType:'hardware', subTypes:["Chip"], installCost:2,
	recurringCredits: 1,
...
coreSet[6] = { title:'Grimoire', imageFile:'01006.png', player:runner, cardType:'hardware', subTypes:["Console"], installCost:3, unique:true, memoryUnits:2,
	cardInstalled: {
		Resolve: function(card){
			if (CheckCardType(card,["program"]))
			{
...
coreSet[7] = { title:'Corroder', imageFile:'01007.png', player:runner, cardType:'program', subTypes:["Icebreaker","Fracter"], memoryCost:1, installCost:2, strength:2,
	strengthBoost:0,
	modifyStrength: {
		Resolve: function(card){
...
coreSet[8] = { title:'Datasucker', imageFile:'01008.png', player:runner, cardType:'program', subTypes:["Virus"], installCost:1, memoryCost:1,
	strengthReduce:0,
	modifyStrength: {
		Resolve: function(card){
...
coreSet[9] = { title:'Djinn', imageFile:'01009.png', player:runner, cardType:'program', subTypes:["Daemon"], installCost:2, memoryCost:1,
			...
			Resolve: function(params) {
				...
				SpendCredits(runner,1,"using",this,function(){
					var choices = ChoicesArrayCards(runner.stack,function(card){
						if (CheckCardType(card,["program"])) return CheckSubType(card,"Virus");
					...
					DecisionPhase(runner,choices,decisionCallback,null,"Djinn: Search",this);
...
coreSet[10] = { title:'Medium', imageFile:'01010.png', player:runner, cardType:'program', subTypes:["Virus"], installCost:3, memoryCost:1,
		accessAdditional:0,
		runSuccessful: {
			Enumerate: function(){
			...
			Resolve: function(){
				...
				var choices = [];
				for (var i=0; i<this.virus; i++) //each virus counter AFTER the first
				{
					if (i==0) choices.push({num:0, label:"Medium: Access no additional cards"});
					else if (i==1) choices.push({num:1, label:"Medium: Access 1 additional card"});
					else choices.push({num:i, label:"Medium: Access "+i+" additional cards"});
				}
				function decisionCallback(params) {
...
coreSet[11] = { title:'Mimic', imageFile:'01011.png', player:runner, cardType:'program', subTypes:["Icebreaker","Killer"], memoryCost:1, installCost:3, strength:3,
	abilities:[
		{
...
coreSet[12] = { title:'Parasite', imageFile:'01012.png', player:runner, player:runner, cardType:'program', subTypes:["Virus"], installCost:2, memoryCost:1,
...
coreSet[13] = { title:'Wyrm', imageFile:'01013.png', player:runner, cardType:'program', subTypes:["Icebreaker","AI"], memoryCost:1, installCost:1, strength:1,
	strengthBoost:0,
	strengthReduce:0,
	modifyStrength: {
		Resolve: function(card){
			if (!CheckEncounter()) return 0;
			if (card == this) return this.strengthBoost;
			else if (card == attackedServer.ice[approachIce]) return this.strengthReduce;
			return 0; //no modification to strength
		}
	},
	abilities:[
...
coreSet[14] = { title:'Yog.0', imageFile:'01014.png', player:runner, cardType:'program', subTypes:["Icebreaker","Decoder"], memoryCost:1, installCost:5, strength:3,
	abilities:[
		{
			...
			Resolve: function(params) {
				SpendCredits(runner,0,"using",this,function(){
					Break(params.subroutine);
				},this);
			}
		}
...
coreSet[15] = { title:'Ice Carver', imageFile:'01015.png', player:runner, cardType:'resource', subTypes:["Virtual"], installCost:3, unique:true,
	modifyStrength: {
		Resolve: function(card){
			if (!CheckEncounter()) return 0;
...
coreSet[16] = { title:'Wyldside', imageFile:'01016.png', player:runner, cardType:'resource', subTypes:["Seedy"], installCost:3, unique:true,
	runnerTurnBegin: {
		Resolve: function() {
			Draw(runner,2);
			LoseClicks(runner,1);
...
coreSet[17] = { title:'Gabriel Santiago: Consummate Professional', imageFile:'01017.png', player:runner, cardType:'identity' };
coreSet[20] = { title:'Forged Activation Orders', imageFile:'01020.png', player:runner, player:runner, cardType:'event', subTypes:["Sabotage"], playCost:1,
		Enumerate: function() {
		...
		Resolve: function(params) {
			...
			if ( CheckRez(card,["ice"]) && CheckCredits(RezCost(card),corp,"rezzing",card) )
			{
				var choicesb = [
				...
				function decisionCallbackb(paramsb) {
					...
					if (paramsb.id == 0)
					{
						SpendCredits(corp,RezCost(card),"rezzing",card,function(){
							Rez(card);
						},this);
					}
					else
				...
				DecisionPhase(corp,choicesb,decisionCallbackb,null,"Forged Activation Orders",this); //choose whether to rez or trash
			}
			else
			{
...
coreSet[30] = { title:'Crash Space', imageFile:'01030.png', player:runner, cardType:'resource', subTypes:["Location"], installCost:2,
	recurringCredits: 2,
	canUseCredits: function(doing,card) {
...
coreSet[32] = { title:'Decoy', imageFile:'01032.png', player:runner, cardType:'resource', subTypes:["Connection"], installCost:1,
	addTags: {
		Enumerate: function(){
...
coreSet[33] = { title:'Kate "Mac" McCaffrey: Digital Tinker', imageFile:'01033.png', player:runner, link:1, cardType:'identity', subTypes:["Natural"],
	usedThisTurn: false,
	modifyInstallCost: {
		Resolve: function(card){
	...
	cardInstalled: {
		Resolve: function(card){
			...
			if (CheckCardType(card,["program","hardware"])) this.usedThisTurn = true;
		}
	},
	runnerTurnBegin: {
		Resolve: function() {
...
coreSet[34] = { title:'Diesel', imageFile:'01034.png', player:runner, cardType:'event', playCost:0,
		Resolve: function(params) {
			Draw(runner,3);
		}
};
coreSet[35] = { title:'Modded', player:runner, cardType:'event', playCost:0 };
coreSet[38] = { title:'Akamatsu Mem Chip', imageFile:'01038.png', player:runner, cardType:'hardware', subTypes:["Chip"], installCost:1, memoryUnits:1 };
coreSet[39] = { title:'Rabbit Hole', player:runner, cardType:'hardware', installCost:2 };
coreSet[40] = { title:'The Personal Touch', imageFile:'01040.png', player:runner, cardType:'hardware', subTypes:["Mod"], installCost:2,
	installOnlyOn: function(card){
...
coreSet[41] = { title:'The Toolbox', imageFile:'01041.png', player:runner, cardType:'hardware', subTypes:["Console"], installCost:9, link:2, memoryUnits:2,
	recurringCredits: 2,
	canUseCredits: function(doing,card) {
		if (doing=="using")
		{
...
coreSet[43] = { title:'Gordian Blade', imageFile:'01043.png', player:runner, cardType:'program', subTypes:["Icebreaker","Decoder"], memoryCost:1, installCost:4, strength:2,
		strengthBoost:0,
		modifyStrength: {
			Resolve: function(card){
				if (card == this)	return this.strengthBoost;
...
coreSet[45] = { title:'Net Shield', imageFile:'01045.png', player:runner, cardType:'program', installCost:2, memoryCost:1,
	...
	netDamage: {
		...
		Resolve: function(){
			...
			var choices = [];
			choices.push({id:0, label:"1[c]: Prevent the first net damage this turn"});
			choices.push({id:1, label:"Continue"});
			function decisionCallback(params)
			{
				if (params.id==0)
			...
			DecisionPhase(runner,choices,decisionCallback,null,"Net Shield",this);
		}
	}
};
coreSet[48] = { title:'Sacrificial Construct', imageFile:'01048.png', player:runner, cardType:'resource', subTypes:["Remote"], installCost:0,
	trash: {
		Enumerate: function(){
		...
		Resolve: function(){
			var choices = [];
			choices.push({id:0, label:"Trash: Prevent an installed program or an installed piece of hardware from being trashed"});
			choices.push({id:1, label:"Continue"});
			function decisionCallback(params)
			{
				if (params.id==0)
			...
			DecisionPhase(runner,choices,decisionCallback,null,"Sacrificial Construct",this);
		}
	}
};
coreSet[49] = { title:'Infiltration', imageFile:'01049.png', player:runner, cardType:'event', playCost:0,
		Enumerate: function() {
			var ret = [{id:0, label:"Gain 2 credits"}];
			if (ChoicesInstalledCards(corp,function(card){
				if (card.rezzed==true) return false; //only include unrezzed installed cards
				return true;
			}).length > 0) ret.push({id:1, label:"Expose 1 card"});
			return ret;
		},
		Resolve: function(params) {
			if (params.id == 0)
			...
			else if (params.id == 1)
			{
				...
				DecisionPhase(runner,choices,decisionCallback,null,"Infiltration",this); //choose card to expose
			}
		}
};
coreSet[50] = { title:'Sure Gamble', imageFile:'01050.png', player:runner, cardType:'event', playCost:5,
		Resolve: function(params) {
			GainCredits(runner,9);
		}
};
coreSet[51] = { title:'Crypsis', imageFile:'01051.png', player:runner, cardType:'program', subTypes:["Icebreaker","AI","Virus"], memoryCost:1, installCost:5, strength:0, virus:0,
		...
		abilities:[
			...
			},
			{
				...
				Resolve: function(params) { SpendClicks(runner,1); AddCounters(this,"virus",1);	Log("Placed 1 virus counter on Crypsis"); }
			}
		],
		encounterEnds: {
			Resolve: function() {
				this.strengthBoost = 0;
				if (this.cardLocation == runner.rig.programs) //paying the virus/trash is only relevant if installed still
				{
...
coreSet[52] = { title:'Access to Globalsec', imageFile:'01052.png', player:runner, cardType:'resource', subTypes:["Link"], installCost:1, link:1 };
coreSet[53] = { title:'Armitage Codebusting', imageFile:'01053.png', player:runner, cardType:'resource', subTypes:["Job"], installCost:1,
		cardInstalled: {
			Resolve: function(card){
...
coreSet[57] = { title:'Aggressive Secretary', imageFile:'01057.png', player:corp, cardType:'asset', subTypes:["Ambush"], rezCost:0, canBeAdvanced:true, trashCost:0,
	cardAccessed: {
		Resolve: function(card){
			if (card == this) {
				...
				var choices = [
					{id:0, label:"2[c]: Trash 1 program for each advancement token on Aggressive Secretary"},
					{id:1, label:"Continue"}
				];
				function decisionCallback(params) {
					if (params.id == 0)
					{
						SpendCredits(corp,2,"",this,function(){
							TrashPrograms(this.advancement);
						},this);
					}
				}
				DecisionPhase(corp,choices,decisionCallback,null,"Aggressive Secretary",this);
			}
		}
	}
};
coreSet[60] = { title:'Shipment from MirrorMorph', imageFile:'01060.png', player:corp, cardType:'operation', playCost:1,
		Resolve: function(params) {
...
coreSet[61] = { title:'Heimdall 1.0', imageFile:'01061.png', player:corp, cardType:'ice', rezCost:8, strength:6, subTypes:["Barrier","Bioroid","AP"],
		cardEncountered: {
			Resolve: function(card){
				if (card == this) {
					...
					for (var i=0; i<this.subroutines.length; i++)
					{
						...
						if (!subroutine.broken) choices.push({subroutine:subroutine, label:"[click]: Break "+subroutine.text});
					}
					...
					DecisionPhase(runner,choices,decisionCallback,null,"Heimdall 1.0",this);
				}
			}
		},
		subroutines:[
...
coreSet[63] = { title:'Viktor 1.0', imageFile:'01063.png', player:corp, cardType:'ice', rezCost:3, strength:3, subTypes:["Code Gate","Bioroid","AP"],
		cardEncountered: {
			Resolve: function(card){
				if (card == this) {
					if (!CheckClicks(1,runner)) return;
					...
					DecisionPhase(runner,choices,decisionCallback,null,"Viktor 1.0",this);
				}
			}
		},
		subroutines:[
...
coreSet[64] = { title:'Rototurret', imageFile:'01064.png', player:corp, cardType:'ice', rezCost:4, strength:0, subTypes:["Sentry","Destroyer"],
		subroutines:[
			{
...
coreSet[65] = { title:'Corporate Troubleshooter', imageFile:'01065.png', player:corp, cardType:'upgrade', subTypes:["Connection"], rezCost:0, trashCost:2 };
coreSet[67] = { title:'Jinteki: Personal Evolution', imageFile:'01067.png', player:corp, cardType:'identity', subTypes:["Megacorp"],
	scored: {
		Resolve: function() {
...
coreSet[68] = { title:'Nisei MK II', imageFile:'01068.png', player:corp, cardType:'agenda', subTypes:["Initiative"], agendaPoints:2, advancementRequirement:4,
	agenda:0,
	scored: {
		Resolve: function() {
...
coreSet[69] = { title:'Project Junebug', imageFile:'01069.png', player:corp, cardType:'asset', subTypes:["Ambush","Research"], rezCost:0, canBeAdvanced:true, trashCost:0,
	cardAccessed: {
		Resolve: function(card){
			if (card == this) {
				//cannot be used if no advancement, see here: http://ancur.wikia.com/wiki/Activate_Blank_%22When_Accessed%22_Ability_Ruling
				...
				var choices = [
					{id:0, label:"1[c]: Do 2 net damage for each advancement token on Project Junebug"},
					{id:1, label:"Continue"}
				];
				...
				function decisionCallback(params) {
					if (params.id == 0)
					{
						SpendCredits(corp,1,"",this,function(){
							NetDamage(2*this.advancement);
						},this);
					}
				}
				DecisionPhase(corp,choices,decisionCallback,null,"Project Junebug",this);
			}
		}
	}
};
coreSet[70] = { title:'Snare!', imageFile:'01070.png', player:corp, cardType:'asset', subTypes:["Ambush"], rezCost:0, trashCost:0,
	cardAccessed: {
		Resolve: function(card){
			if (card == this) {
				...
				function abilityPart() {
					...
					if ((!CheckCredits(4,corp,"",this))||(this.cardLocation == corp.archives.cards)) return;
					var choices = [
					...
					function decisionCallback(params) {
						if (params.id == 0)
						{
							SpendCredits(corp,4,"",this,function(){
								NetDamage(3,function(cardsTrashed){ AddTags(1); },this);
							},this);
						}
					}
				...
				if (this.cardLocation == corp.RnD.cards) Reveal(this,abilityPart,this);
...
coreSet[71] = { title:'Zaibatsu Loyalty', imageFile:'01071.png', player:corp, cardType:'asset', rezCost:0, trashCost:4,
	expose: {
		Enumerate: function(){
		...
		Resolve: function(){
			...
			if (!this.rezzed) //if Zaibatsu Loyalty is not rezzed, give corp an opportunity to rez it
			{
				var currentRezCost = RezCost(this);
				if (CheckCredits(currentRezCost,corp,"rezzing",this)) choices.push({id:0, label:currentRezCost+"[c]: Rez Zaibatsu loyalty"});
				choices.push({id:1, label:"Continue"});
				decisionCallback = function(params)
				{
					if (params.id==0)
					{
						SpendCredits(corp,RezCost(this),"rezzing",this,function(){
							Rez(this);
			...
			else //if Zaibatsy Loyalty is rezzed, its ability can be used
			{
				if (CheckCredits(1,corp,"using",this)) choices.push({id:0, label:"1[c]: Prevent 1 card from being exposed."});
				choices.push({id:1, label:"Trash: Prevent 1 card from being exposed"});
				...
				decisionCallback = function(params)
				{
					...
					if (params.id==0) SpendCredits(corp,1,"using",this,afterCall,this);
					else if (params.id==1) Trash(this,false,afterCall,this);
					else return;
				}
			}
			DecisionPhase(corp,choices,decisionCallback,null,"Zaibatsu Loyalty",this);
		},
		availableWhenInactive: true
	}
};
coreSet[72] = { title:'Neural EMP', imageFile:'01072.png', player:corp, cardType:'operation', playCost:2,
	...
	corpTurnBegin:{ //only runs during runner turn can trigger condition for Neural EMP
		Resolve: function() {
...
coreSet[73] = { title:'Precognition', imageFile:'01073.png', player:corp, cardType:'operation', playCost:0,
	cardsToLookAt: null,
	Enumerate: function()
	{
	...
	Resolve: function(params) {
		if (this.cardsToLookAt===null)
		{
			...
			for (var i=0; (i<5)&&(corp.RnD.cards.length>i); i++)
			{
				...
				this.cardsToLookAt.push({returnCard:card, label:"Place "+GetTitle(card,true)+" on top of R&D."});
			}
		}
		if (this.cardsToLookAt.length > 0)
		{
			...
			if (this.cardsToLookAt.length > 0) DecisionPhase(corp,this.cardsToLookAt,this.Resolve,null,"Precognition",this);
			else this.Resolve();
		}
	}
};
coreSet[74] = { title:'Cell Portal', imageFile:'01074.png', player:corp, cardType:'ice', rezCost:5, strength:7, subTypes:["Code Gate","Deflector"],
		subroutines:[
			{
				...
				Resolve: function() {
					...
					Log("Approaching outermost piece of ice protecting "+attackedServer.serverName);
...
coreSet[75] = { title:'Chum', imageFile:'01075.png', player:corp, cardType:'ice', rezCost:1, strength:4, subTypes:["Code Gate"],
...
coreSet[76] = { title:'Data Mine', imageFile:'01076.png', player:corp, cardType:'ice', rezCost:0, strength:2, subTypes:["Trap","AP"],
		subroutines:[
			{
...
coreSet[77] = { title:'Neural Katana', imageFile:'01077.png', player:corp, cardType:'ice', subTypes:["Sentry","AP"], rezCost:4, strength:3,
	subroutines:[
		{
...
coreSet[78] = { title:'Wall of Thorns', imageFile:'01078.png', player:corp, cardType:'ice', subTypes:["Barrier","AP"], rezCost:8, strength:5,
	subroutines:[
		{
...
coreSet[79] = { title:'Akitaro Watanabe', imageFile:'01079.png', player:corp, cardType:'upgrade', subTypes:["Sysop","Unorthodox"], rezCost:1, trashCost:3, unique:true,
	modifyRezCost: {
		Resolve: function(card){
...
coreSet[80] = { title:'NBN: Making News', imageFile:'01080.png', player:corp, cardType:'identity', subTypes:["Megacorp"],
	recurringCredits: 2,
	canUseCredits: function(doing,card) {
...
coreSet[88] = { title:'Data Raven', imageFile:'01088.png', player:corp, cardType:'ice', rezCost:4, strength:4, subTypes:["Sentry","Tracer","Observer"],
		power:0,
		cardEncountered: {
			Resolve: function(card){
				if (card == this) {
					var choices = [{id:0, label:"Take 1 tag"},{id:1, label:"End the run"}];
					function decisionCallback(params) {
						if (params.id == 0) AddTags(1);
						else EndTheRun();
					}
					DecisionPhase(runner,choices,decisionCallback,null,"Data Raven",this);
				}
			}
		},
		subroutines:[
			{
				...
				Resolve: function(params) {
					Trace(3,function(successful){ if (successful) AddCounters(attackedServer.ice[approachIce],"power"); });
				}
			}
		],
		abilities:[
...
coreSet[89] = { title:'Matrix Analyzer', imageFile:'01089.png', player:corp, cardType:'ice', rezCost:1, strength:3, subTypes:["Sentry","Tracer","Observer"],
		cardEncountered: {
			Resolve: function(card){
				if (card == this) {
					var matrixCard = this;
					var matrixAnalyzerPhase = {
						...
						Enumerate: {
							advance: function() {
								if (CheckCredits(1,corp)) return ChoicesInstalledCards(corp,CheckAdvance);
								else return [];
							}
						},
						Resolve: {
							advance: function(params) {
								SpendCredits(corp,1,"",matrixCard,function(){
		...
		subroutines:[
			{
				...
				Resolve: function(params) {
					Trace(2,function(successful){ if (successful) AddTags(1)});
				}
			}
		]
};
coreSet[92] = { title:'SanSan City Grid', imageFile:'01092.png', player:corp, cardType:'upgrade', subTypes:["Region"], rezCost:6, trashCost:5,
...
coreSet[94] = { title:'Hostile Takeover', imageFile:'01094.png', player:corp, cardType:'agenda', subTypes:["Expansion"], agendaPoints:1, advancementRequirement:2,
	scored: {
		Resolve: function() {
...
coreSet[107] = { title:'Private Security Force', imageFile:'01107.png', player:corp, cardType:'agenda', subTypes:["Security"], agendaPoints:2, advancementRequirement:4,
	abilities:[
		{
...
coreSet[108] = { title:'Melange Mining Corp.', imageFile:'01108.png', player:corp, cardType:'asset', rezCost:1, trashCost:1,
		abilities:[
			{
				text: "Gain 7[c]",
				Enumerate: function() {
					if (!CheckActionClicks(3)) return []; //no legal options
					return [{}]; //one legal option, no properties
				},
				Resolve: function(params) { SpendClicks(corp,3); GainCredits(corp,7); }
			}
		]
};
coreSet[109] = { title:'PAD Campaign', imageFile:'01109.png', player:corp, cardType:'asset', subTypes:["Advertisement"], rezCost:2, trashCost:4,
		corpTurnBegin: {
			Resolve: function(){ GainCredits(corp,1); },
			automatic:true //for usability, this is not strict implementation
		},
		text: "Gain 1 credit."
};
coreSet[110] = { title:'Hedge Fund', imageFile:'01110.png', player:corp, cardType:'operation', subTypes:["Transaction"], playCost:5,
		Resolve: function(params) {
			GainCredits(corp,9);
		}
};
coreSet[111] = { title:'Enigma', imageFile:'01111.png', player:corp, cardType:'ice', rezCost:3, strength:2, subTypes:["Code Gate"],
		subroutines:[
			{
...
coreSet[112] = { title:'Hunter', imageFile:'01112.png', player:corp, cardType:'ice', rezCost:1, strength:4, subTypes:["Sentry","Tracer","Observer"],
	subroutines:[
		{
			text: "Trace3 - If successful, give the Runner 1 tag.",
			Resolve: function(params) {
				Trace(3,function(successful){ if (successful) AddTags(1); });
			}
		}
	]
};
coreSet[113] = { title:'Wall of Static', imageFile:'01113.png', player:corp, cardType:'ice', rezCost:3, strength:3, subTypes:["Barrier"],
	subroutines:[
		{
			text: "End the run.",
			Resolve: function() {
