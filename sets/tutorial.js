...
var tutorial = [];
tutorial[0] = { title:'The Catalyst (without advanced cards)', imageFile:'30076.png', player:runner, link:0, cardType:"identity", subTypes:["Natural"],
	hideTags: true,
	...
	tutorialSteps: [{
		...
		str:'Runner 1.3', action:function() {
			TutorialMessage("Draw a card by dragging the top card of your stack to your hand");
			TutorialReplacer = function(input)
			{
				if (input == 'gain') TutorialMessage("Clicking for credits is inefficient.\nRight now it's better to draw a card by dragging it to your hand.");
				else if (input == 'run') TutorialMessage("It can be dangerous to run with too few cards in your hand.\nDraw a card by dragging it to your hand.");
