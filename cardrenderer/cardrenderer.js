...
Cascade:class {
	constructor(cards,xStep,yStep,hostingX,faceDownOffset=0) {
		...
		for (var i=0; i<this.cards.length; i++)
		{
			...
			if (xOffset + cardExtents.l < this.extents.l) this.extents.l = xOffset + cardExtents.l;
			if (xOffset + cardExtents.r > this.extents.r) this.extents.r = xOffset + cardExtents.r;
			if (yOffset + cardExtents.t < this.extents.t) this.extents.t = yOffset + cardExtents.t;
			if (yOffset + cardExtents.b > this.extents.b) this.extents.b = yOffset + cardExtents.b;
			...
			if (typeof(card.hostedCards) !== 'undefined')
			{
				for (var j=0; j<card.hostedCards.length;j++)
				{
					...
					if (xOffset + hostedCardExtents.l < this.extents.l) this.extents.l = xOffset + hostedCardExtents.l;
					if (xOffset + hostedCardExtents.r > this.extents.r) this.extents.r = xOffset + hostedCardExtents.r;
					if (yOffset + hostedCardExtents.t < this.extents.t) this.extents.t = yOffset + hostedCardExtents.t;
					if (yOffset + hostedCardExtents.b > this.extents.b) this.extents.b = yOffset + hostedCardExtents.b;
				}
			}
			xOffset+=this.xStep;
			yOffset+=this.yStep;
	...
	Apply(app,x,y,anchorX,anchorY,hostingX,faceDownOffset=0) //anchor relates the Cascade position to its extents (default puts anchor at anchor of first card)
	{
		var xOffset = x;
		var yOffset = y;
		//s.t. if anchorX = 0, xOffset = -this.extents.l and if anchorX = 1, xOffset = -this.extents.r
		if (typeof(anchorX)!=='undefined') xOffset -= this.width*anchorX + this.extents.l;
		if (typeof(anchorY)!=='undefined') yOffset -= this.height*anchorY + this.extents.t;
...
Counter:class {
	constructor(app,texture,style,address,key,scale,hideWhenZero,clickCallback) {
		this.app = app;
		...
		this.particleContainer.scale.x = this.particleContainer.scale.y = 1.0/scale; //undo sprite scaling for particles
		...
		//set scale
		this.sprite.scale.x = this.richText.scale.x = this.sprite.scale.y = this.richText.scale.y = scale;
		...
		app.ticker.add(function(delta) {
			if (typeof(this.glowSprite) !== 'undefined')
			{
				...
				if (availability > 0)
				{
					...
					if (this.sprite.hover) this.glowSprite.tint = parseInt("D800FF",16); //purple
					else this.glowSprite.tint = parseInt("FFFFFF",16); //white
				}
				else if (availability == -2) //not interactive, just to highlight something
				{
					this.glowSprite.visible = true;
			...
			if (this.animateCountdown <= 0.0)
			{
				if (this.incrementers.length > 0)
				{
					...
					if (newValue > oldValue) cardRenderer.ParticleEffect(this.particleContainer,particleSystems.addcredit);
					else if (newValue < oldValue) cardRenderer.ParticleEffect(this.particleContainer,particleSystems.removecredit);
...
Card:class {
	...
	constructor(app,card,front,back,glow,strengthInfo={texture:null,num:0,ice:false,cost:null},textStyle) {
		this.app = app;
		...
		this.defaultMask = new PIXI.Graphics().beginFill(0xffffff).drawRoundedRect(dMD.x, dMD.y, dMD.w, dMD.h, dMD.r).endFill();
		...
		this.dummyDefaultMask = new PIXI.Graphics().beginFill(0xffffff).drawRoundedRect(dMD.x, dMD.y, dMD.w, dMD.h, dMD.r).endFill();
		...
		this.unmaskedDimensions = { x:bounds.x, y:bounds.y, w:bounds.width, h:bounds.height, r:5 };
		var uMD = this.unmaskedDimensions;
		this.sprite.hitArea = new PIXI.RoundedRectangle(uMD.x, uMD.y, uMD.w, uMD.h, uMD.r);
		...
		if (strengthInfo.texture != null)
		{
			...
			if (strengthInfo.ice) //i.e. card is ice (not icebreaker)
			{
				var iMD = this.iceMaskDimensions;
				
				this.iceMask = new PIXI.Graphics().beginFill(0xffffff).drawRoundedRect(iMD.x, iMD.y, iMD.w, iMD.h, iMD.r).endFill();
				this.iceMask.visible = false;
				this.sprite.addChild(this.iceMask);
				
				this.dummyIceMask = new PIXI.Graphics().beginFill(0xffffff).drawRoundedRect(iMD.x, iMD.y, iMD.w, iMD.h, iMD.r).endFill();
			...
			app.ticker.add(function(delta) {
				...
				if (this.strengthAnimateCountdown <= 0.0)
				{
					if (this.strengthIncrementers.length > 0)
					{
						...
						if (newValue > oldValue) cardRenderer.ParticleEffect(this.particleContainer,particleSystems.strengthup);
						else if (newValue < oldValue) cardRenderer.ParticleEffect(this.particleContainer,particleSystems.strengthdown);
		...
		{
			this.printedCost = 0;
			if (typeof(this.card.rezCost) !== 'undefined') this.printedCost = this.card.rezCost;
			else if (typeof(this.card.playCost) !== 'undefined') this.printedCost = this.card.playCost;
			else if (typeof(this.card.installCost) !== 'undefined') this.printedCost = this.card.installCost;
			...
			this.costParticleContainer.scale.x = this.costParticleContainer.scale.y = 2.0; //undo sprite scaling for particles
			...
			app.ticker.add(function(delta) {
				...
				if (typeof(this.card.rezCost) !== 'undefined') this.targetCost = RezCost(this.card);
				//else if (typeof(this.card.playCost) !== 'undefined') this.targetCost = PlayCost(this.card); //nothing modifies this at the moment
				else if (typeof(this.card.installCost) !== 'undefined') this.targetCost = InstallCost(this.card);
				//add incrementers to animate increase/decrease
				...
				if (this.costAnimateCountdown <= 0.0)
				{
					if (this.costIncrementers.length > 0)
					{
						...
						if (newValue > oldValue) cardRenderer.ParticleEffect(this.costParticleContainer,particleSystems.costup);
						else if (newValue < oldValue) cardRenderer.ParticleEffect(this.costParticleContainer,particleSystems.costdown);
		...
		this.UpdateGlow = function() {
			...
			if (this.availability == -3) //accessing
			{
				this.glowSprite.visible = true;
			...
			{
				this.glowSprite.visible = true;
				if (this.availability == 3) //special case - toggled as selection 
				{
				...
				else if (this.hover)
				{
					...
					{
						if (this.availability == 2) this.glowSprite.tint = parseInt("FFD800",16); //yellow (to indicate it won't immediately activate)
			...
			{
				if ((pixi_draggingData !== null)||(OptionsAreOnlyUniqueServers())) //i.e. something is being dragged or a server selection is being made
				{	
					var server = null;
					this.glowSprite.visible = false;
					...
					if (validServer)
					{
						...
						{
							if (pixi_playThreshold(pixi_draggingCard)) this.glowSprite.tint = parseInt("D800FF",16); //purple
						}
					}
					else //not a server? maybe a potential host
					{
						if (CardIsValidHostOption(this.card))
						{
							...
							if (pixi_playThreshold(pixi_draggingCard) && this.isClosestHost)
							{
								this.glowSprite.tint = parseInt("D800FF",16); //purple
							}
						}
        ...
        this.sprite.on('pointerdown', pixi_onDragStart)
		...
		app.ticker.add(function(delta) {
			...
			var useCropped = ((forceCropped||(!this.zoomed))&&(!isApproachEncounter)&&(pixi_draggingCard !== this));			
			if (useCropped)
			{
				...
				if (this.strengthSprite != null) //icebreaker
				{
				...
				this.sprite.hitArea = new PIXI.RoundedRectangle(hAD.x+maskOffset.x, hAD.y+maskOffset.y, hAD.w, hAD.h, hAD.r);
			}
			else //zoomed or being approached/encountered
			{
				...
				if (this.strengthSprite != null) //ice or icebreaker
				{
					if (typeof(this.iceMask) !== 'undefined')
					{
						this.iceMask.visible = false;
						this.dummyIceMask.visible = false;
						if (GetApproachEncounterIce() == this.card) //being approached/encountered (or moving on from)
						{
				...
				this.sprite.hitArea = new PIXI.RoundedRectangle(hAD.x, hAD.y, hAD.w, hAD.h, hAD.r);
			...
			if ((this.hover)&&(this.zoomed)&&(!this.sprite.dragging))
			{
				...
				if (this.minZoomLoops < 1)
				{
					...
					var isHit = this.app.renderer.plugins.interaction.hitTest(cardRenderer.MousePosition());
					this.sprite.interactive = true;
					if (isHit)
					{
			...
			var scaleDiff = 0;
			if ((((this.zoomed === true)&&(!forceCropped))||(this.sprite.dragging === true))&&(this.card.faceUp||this.canView))
			{
				destRot = 0;
				if (viewingPlayer == corp)
				{
					if ((this.sprite.dragging === true)&&(this.card.cardType=="ice")) destRot = Math.PI*0.5;
			...
			var flipSpeed = 0.1;
			if (showFace && (this.flipProgress < 1.0)) //flipping face up
			{
				this.flipProgress += flipSpeed*delta; 
				if (this.flipProgress > 0.0) this.SetTextureToFront();
			}
			else if (!showFace && (this.flipProgress > -1.0)) //flipping face down
			{
				this.flipProgress -= flipSpeed*delta; 
				if (this.flipProgress < 0.0) this.SetTextureToBack();
			}
			else //in desired state already
			{
				if (showFace)
				{
			...
			if ((this.canView)&&(this.flipProgress < 0.0)) this.knownSprite.visible = true;
			else this.knownSprite.visible = false;
			...
			if (scalingSpeed > 0)
			{
				if (this.sprite.scale.y > scalingratio) this.sprite.scale.y = scalingratio;
			}
			else
			{
				if (this.sprite.scale.y < scalingratio) this.sprite.scale.y = scalingratio;
			...
			var hideDummy = true;
			if (forceCropped) //keep immovable cards where they are when unzoomed (prevents glitch when near edge of screen due to zoom realignment)
			{
				this.sprite.x = this.destinationPosition.x;
				this.sprite.y = this.destinationPosition.y;
			}
			else if (this.zoomed) //apply some last positionings
			{				
				//on mobile, move the card elsewhere other than your finger
				if (scalingratio > 0.5) //i.e., actually zoomed
				{
					hideDummy = false;
					if (pixi_holdZoom) //i.e., on mobile
					{
						var arbitraryExtraSpace = 100;
						if (this.storedPosition.x < cardRenderer.app.renderer.width*0.5) this.sprite.x = cardRenderer.app.renderer.width - (0.5*this.sprite.width) - arbitraryExtraSpace;
						else this.sprite.x = (0.5*this.sprite.width) + arbitraryExtraSpace;
						this.sprite.y = 0.5*cardRenderer.app.renderer.height;
					}
				}
				//prevent card going off the screen
				if (this.sprite.x - (0.5*this.sprite.width) < 0) this.sprite.x = 0.5*this.sprite.width;
				if (this.sprite.x + (0.5*this.sprite.width) > cardRenderer.app.renderer.width) this.sprite.x = cardRenderer.app.renderer.width-0.5*this.sprite.width;
				if (this.sprite.y - (0.5*this.sprite.height) < 0) this.sprite.y = 0.5*this.sprite.height;
				if (this.sprite.y + (0.5*this.sprite.height) > cardRenderer.app.renderer.height) this.sprite.y = cardRenderer.app.renderer.height-0.5*this.sprite.height;				
			}
	...
	Clone() {
		return new CardRenderer.Card(this.app, this.frontTexture, this.backTexture, this.glowTextures);
	}
	...
	ToggleZoom()
	{
		...
		else if (this.zoomed === true) //unzoom
		{
			this.sprite.parent.setChildIndex(this.sprite,this.sprite.originalIndex);
			this.glowSprite.parent.setChildIndex(this.glowSprite,this.glowSprite.originalIndex);
			...
			if ((this == pixi_holdCard)&&pixi_holdZoom) //i.e. touch to zoom (we visually moved the card into another place to avoid finger obstructing)
			{
		...
		else if (this.faceUp||this.canView||(this.availability==2)||(this == pixi_holdCard)) //zoom
		{
			this.storedPosition.x = this.sprite.x;
			this.storedPosition.y = this.sprite.y;
			this.storedRotation = this.sprite.rotation;			

			this.sprite.originalIndex = this.sprite.parent.getChildIndex(this.sprite);
			this.glowSprite.originalIndex = this.glowSprite.parent.getChildIndex(this.glowSprite);
	...
	OnClick()
	{
		if (this.card.availability == 2) //i.e. drag and drop to choose
		{
			if (pixi_playThreshold(this.card))
			{
				if (ResolveClick(this.card))
				{
					...
					if (this.card.card.player == corp) this.card.destinationRotation = Math.PI;
		...
		else if ((this.card.availability == 1)||(this.card.availability == 3)) //i.e. click to interact (3 means card is toggled on, click toggles it off)
		{
			this.pointerout(); //clear highlight
		...
		if (viewingPile == null)
		{
			if (((this.card.card.cardLocation == corp.archives.cards)&&(viewingPlayer==corp))||((this.card.card.cardLocation == runner.heap)&&(viewingPlayer==runner)))
			{
	...
	Extents() {
		...
		//now compute extents
		var oldTexture = null;
		if (!this.sprite.texture.valid) //front texture not loaded, use back
		{
...
Renderer:class {
	...
	constructor(resizeCallback) {
		...
		this.iceGlow = new PIXI.Sprite(PIXI.Texture.fromImage('images/ice_white.png'));
		...
		this.UpdateGlow = function(card,glow,x=null,y=null) { //card is a CardRenderer.Card
			...
			else if (card == null) //approaching a server
			{
				this.iceGlow.visible = true;
				this.iceGlow.x = x;
				this.iceGlow.y = y;
				this.iceGlow.scale.x = 0.4;
				if (accessList.length > 0) this.iceGlow.tint = parseInt("0000FF",16); //accessing (blue)
				else if (movement)
				{
			...
			else //approaching ice
			{
				this.iceGlow.visible = true;
				this.iceGlow.x = card.sprite.x;
				this.iceGlow.y = card.sprite.y;
				this.iceGlow.scale.x = 0.5;
				if (glow == 1) this.iceGlow.tint = parseInt("FF0000",16); //encountering (red)
				else if (glow == 2) this.iceGlow.tint = parseInt("FFFF00",16); //interfacing (yellow)
				else if (glow == 3)
				{
					this.iceGlow.tint = parseInt("0000FF",16); //all broken/movement (blue)
					this.iceGlow.rotation = Math.PI;
				}
				else this.iceGlow.tint = parseInt("0000FF",16); //approaching (blue) by convention indicates 'in progress, not a choice to click'
			}
		}
		
		//and empty&unprotected archives/new remote indicators
		var indicatorsTexture = PIXI.Texture.fromImage('images/glow_outline_dashed_cropped.png');
		...
		this.app.ticker.add(function(delta) {
			...
			this.archivesIndicator.visible = false;
			if (ServerIsValidOption(corp.archives)&&(corp.archives.cards.length==0)&&(corp.archives.root.length==0)&&(corp.archives.ice.length==0)) //show indicator only if archives is not empty-and-unprotected
			{
				this.archivesIndicator.visible = true;
				if (MouseIsOverServer(corp.archives))
				{
					if (pixi_playThreshold(pixi_draggingCard)) this.archivesIndicator.tint = parseInt("D800FF",16); //purple
					else this.archivesIndicator.tint = parseInt("FFFFFF",16); //white
				}
				else this.archivesIndicator.tint = parseInt("FFFFFF",16); //white
			}				
			if (corp.archives.xStart == corp.archives.xEnd) this.archivesIndicator.x = 0.5*(corp.archives.xStart + corp.archives.xEnd) - 100; //the subtract is arbitrary for aesthetics (spacing from RnD)
			else this.archivesIndicator.x = 0.5*(corp.archives.xStart + corp.archives.xEnd); //the subtract is arbitrary for aesthetics (spacing from RnD)
			...
			if (ServerIsValidOption(null))
			{
				...
				{
					if (pixi_playThreshold(pixi_draggingCard)) this.newRemoteIndicator.tint = parseInt("D800FF",16); //purple
			...
			if ((pixi_draggingData !== null)||(OptionsAreOnlyUniqueServers())) //i.e. something is being dragged or a server selection is being made
			{
				...
				if (pixi_playThreshold(pixi_draggingCard)) serverTint = parseInt("D800FF",16); //purple
				...
				for (var i=0; i<validOptions.length; i++)
				{
					if (typeof(validOptions[i].server) !== 'undefined')
					{
						if (MouseIsOverServer(validOptions[i].server))
						{
							...
							if (validOptions[i].server == null) this.serverSelector.x = this.newRemoteIndicator.x;
							else this.serverSelector.x = (validOptions[i].server.xStart + validOptions[i].server.xEnd)*0.5;
							this.serverText.text = ServerName(validOptions[i].server,true,true); //first true ignores remote numbers, second removes indefinite article
				...
				if (CheckEncounter())
				{
					...
					for (var i=0; i<installedRunnerCards.length; i++)
					{
						if (CheckSubType(installedRunnerCards[i],"Icebreaker"))
						{
							...
							{
								//just check type manually for now. later could make an interface utility approach used in common by breakers
								if (CheckSubType(installedRunnerCards[i],"AI"))	breakers.push(installedRunnerCards[i]);
								else if (CheckSubType(installedRunnerCards[i],"Decoder")&&CheckSubType(encounteredIce,"Code Gate"))	breakers.push(installedRunnerCards[i]);
								else if (CheckSubType(installedRunnerCards[i],"Killer")&&CheckSubType(encounteredIce,"Sentry"))	breakers.push(installedRunnerCards[i]);
								else if (CheckSubType(installedRunnerCards[i],"Fracter")&&CheckSubType(encounteredIce,"Barrier")) breakers.push(installedRunnerCards[i]);
							}
						}
					}
					if (breakers.length > 0)
					{
						...
						if ((this.currentBreakerIdx > breakers.length-1)||(this.currentBreakerIdx < 0)||(sqDistToIce < 10000)) //need new target or else have moved close enough to cycle to next
						{
							this.currentBreakerIdx++;
							if (this.currentBreakerIdx > breakers.length-1) this.currentBreakerIdx=0;
						...
						else //animate closer
						{
							...
							var deltaSpeedInvDist = delta*interfacerSpeed/Math.sqrt(sqDistToIce);
		...
		this.interfacer.emitter = new PIXI.particles.Emitter(this.interfacer, [PIXI.Texture.fromImage('images/particle.png')], particleSystems.interface);
		...
		{
			pixi_mousePosition = event.data.global; //since this event is in the main renderer, no need to convert local coordinates
		...
		this.app.renderer.plugins.interaction.on( 'pointerup', function(event) 
		{
			...
			if (OptionsAreOnlyUniqueServers())
			{
				for (var i=0; i<validOptions.length; i++)
				{
					if (MouseIsOverServer(validOptions[i].server)) ResolveClick(validOptions[i].server);
				}
			}
		});
	}
	...
	CreateCard(card,front,back,glow,strengthInfo) {
		return new CardRenderer.Card(this.app,card,front,back,glow,strengthInfo,this.counterStyle);
	}
	...
	{
		var ret = new CardRenderer.Counter(this.app,texture,this.counterStyle,address,key,scale,hideWhenZero,clickCallback);
	...
	UpdateCounters()
	{
		for (var i=0; i<this.counters.length;i++)
		{
			...
			if (this.counters[i].sprite.parent != this.app.stage)  unrotation += this.counters[i].sprite.parent.rotation;
	...
	{
		var emitter = new PIXI.particles.Emitter(container, [PIXI.Texture.fromImage('images/particle.png')], effect);
...
function pixi_onDragMove(event)
{
    ...
    pixi_mousePosition = event.data.getLocalPosition(this.parent);
	var diff = { x:pixi_mousePosition.x-pixi_mouseStart.x, y:pixi_mousePosition.y-pixi_mouseStart.y };
...
function pixi_playThreshold(cardToPlay)
{
	if (cardToPlay != null)
	{
		if ((executingCommand=="trash")||(executingCommand=="discard")) //trash before install or discard at end of turn
		{
			...
			return (pixi_SqDistToPile(pile) < 50000); //arbitrary to feel right
		}
		...
		if (typeof(cardToPlay.card.installOnlyOn) === 'function') //install a card that needs to be hosted
		{
			...
			var validHosts = ChoicesInstalledCards(null,cardToPlay.card.installOnlyOn); //null means both players
			...
			for (var i=0; i<validHosts.length; i++)
			{
				if (closestHost == validHosts[i].card.renderer) validHosts[i].card.renderer.isClosestHost = true;
