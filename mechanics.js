...
function MakeRun(server) {
	...
	if (attackedServer.ice.length > 0) ChangePhase(phases.runApproachIce); //(Nisei 2021 1.4.1 sends to 2.1)
			...
			if (card.unique==true)
			{
				...
				for (var i=0; i<installedCards.length; i++)
				{
					if ((installedCards[i] != card)&&installedCards[i].rezzed)
					{
						...
						{
							Log(GetTitle(card)+" is unique, the older copy will be unpreventably trashed.");
...
function TrashAccessedCard(canBePrevented)
{
	...
	Trash(accessingCard,canBePrevented,function(){ ResolveAccess(originalLocation); });
}
 ...
 */
function Install(installingCard,destination=null,ignoreAllCosts=false,position=null,returnToPhase=true,onInstallResolve,context,onCancelResolve)
{
	var oldLocation = installingCard.cardLocation; //in case of cancel
	var oldPhase = currentPhase; //in case of cancel
 	MoveCard(installingCard, installingCard.player.resolvingCards);
 	...
 	var installTrashPhase = {
 		Enumerate: {
 			trash: function() {
				...
				if (installingCard.cardType == "program")
				{
					//for usability we will skip trashing if there is enough MU left
 					if ((installingCard.cardType=='program')&&typeof(installingCard.memoryCost !== 'undefined'))
 					{
 						if (installingCard.memoryCost + InstalledMemoryCost(destination) <= MemoryUnits(destination)) return [];
 					}
					//but we'll leave this line here just in case, or for later
					return ChoicesInstalledCards(runner,function(card){ return CheckCardType(card,["program"]); }); //The Runner can choose to trash any number of his installed programs at the beginning of an install program action. [Core rulebook]
				}
 			...
 			n: function() {
 				if (installingCard.player==corp)
 				{
 					if (!CheckCredits(InstallCost(installingCard,destination,ignoreAllCosts,position),corp,"installing",installingCard)) return []; //can't afford to n yet
 					if ((installingCard.cardType == 'agenda')||(installingCard.cardType == 'asset'))
 					{
 						var cardlist = InstallDestination(installingCard,destination);
 						for (var i=0; i<cardlist.length; i++)
 						{
 							if ((cardlist[i].cardType == 'agenda')||(cardlist[i].cardType == 'asset')) return []; //only one asset/agenda allowed
 						}
 					}
					else if (CheckSubType(installingCard,"Region")) //upgrade subtype
					{
						var cardlist = InstallDestination(installingCard,destination);
 						for (var i=0; i<cardlist.length; i++)
 						{
		...
		Cancel: {
			trash: function () {
				...
				if (typeof(onCancelResolve) === 'function') onCancelResolve.call(context);
				Cancel();
				Render();
			}
		},
 		Resolve: {
 			trash: function(params)
			{
 			...
 			n: function(){
				...
				if (typeof(onInstallResolve) === 'function') onInstallResolve.call(context);
 				//if clicks were spent, it was done before trash (or as part of callback) so no need to SpendClicks here
 				SpendCredits(installingCard.player,InstallCost(installingCard,destination,ignoreAllCosts,position),"installing",installingCard,function(){
					...
					if (typeof(installingCard.recurringCredits) !== 'undefined') installingCard.credits = installingCard.recurringCredits;
					var outStr = GetTitle(installingCard,true);
					if (CheckCardType(installingCard,["agenda","asset","upgrade"])) outStr="a card in root of "+CardServerName(installingCard,true);
					else if (CheckCardType(installingCard,["ice"])) outStr="ice protecting "+CardServerName(installingCard,true);
	 				Log(PlayerName(installingCard.player)+" installed "+outStr);
					//if unique, old one is immediately and unpreventably trashed (except if facedown, and facedown cards don't count for check)
					if ((typeof(installingCard.unique) !== 'undefined')&&installingCard.faceUp)
					{
							if (installingCard.unique==true)
							{
								...
								for (var i=0; i<installedCards.length; i++)
								{
									if ((installedCards[i] != installingCard)&&installedCards[i].faceUp)
									{	
										if (GetTitle(installedCards[i]) == GetTitle(installingCard))
										{
											Log(GetTitle(installingCard)+" is unique, the older copy will be unpreventably trashed.");
											Trash(installedCards[i],false);
										}
									}
								}
	...
	if (installingCard.player==corp) installTrashPhase.identifier = "Corp Install";
	else if (installingCard.player==runner) installTrashPhase.identifier = "Runner Install";
...
function Play(card,onPlayResolve,context) {
	...
	return DecisionPhase(card.player,choices,resolveCallback,"Playing "+GetTitle(card,true),instruction,card,"continue",cancelCallback);
}
...
function Break(subroutine)
{
	...
	{
		particleSystems.breaksubroutine.spawnRect.x = 209-subroutine.visual.y+(0.5*subroutine.visual.h); //-209 is half card height
		cardRenderer.ParticleEffect(card.renderer.particleContainer, particleSystems.breaksubroutine);
	}
...
{
	if (typeof(triggerable.text) !== 'undefined') Log("Using \""+triggerable.text+"\" on "+GetTitle(card)+":");
...
function TriggerAbility(card, ability, onTriggerResolve, context)
{
	...
	return DecisionPhase(player,choices,resolveCallback,instruction,instruction,card,"continue",cancelCallback);
}
...
function NetDamage(num,afterTrashing,context)
{
	...
	var trashCallback = function()
	{
		...
		{
			if (typeof(afterTrashing) === 'function') afterTrashing.call(context, cardsTrashed);
			return;
		}
...
function MeatDamage(num,afterTrashing,context)
{
	...
	var trashCallback = function()
	{
		...
		intended.meatDamage--;
		Trash(runner.grip[RandomRange(0,runner.grip.length-1)],true,trashCallback);
	}
...
function SpendCredits(player,num,doing="",card=null,afterSpend,context)
{
	...
	if (player==runner)
	{
		...
		if (spendCred_temporary > 0)
		{
			...
			if (spendCred_temporary == 1) Log(PlayerName(player)+" spent one temporary credit"); //TODO specific messages per type?
			else Log(PlayerName(player)+" spent "+spendCred_temporary+" temporary credits");
		}
	}
			...
			if (typeof(activeCards[i].credits) !== 'undefined')
			{
				if (typeof(activeCards[i].canUseCredits) === 'function')
				{
					if (activeCards[i].canUseCredits(doing,card))
					{
						...
						num -= spendCred_card;
						if (spendCred_card == 1) Log(PlayerName(player)+" spent one credit from "+GetTitle(activeCards[i],true));
						else Log(PlayerName(player)+" spent "+spendCred_card+" credits from "+GetTitle(activeCards[i],true));
					}
				}
			}
		}
...
function ShuffleInto(cards,destination)
{
	...
	if (ArrayName(destination)!=="") outText += "into "+destination.displayName;
	Log(outText);
}
...
function LoseCredits(player,num)
{
	...
	if (runner.temporaryCredits > 0)
	{
	 ...
	 else //can only partially cover the cost with bad pub credits
	 {
	...
	else if (numberLost > 1) Log(PlayerName(player)+" lost "+numberLost+" credits");
...
function RunUnsuccessful()
{
	...
	else //just an ordinary phase change
	{
 ...
 */
function Score(card,afterSteal,context) {
	...
	OpportunityForAvoidPrevent(runner,"score",function(){
		...
		TriggeredResponsePhase(corp,"scored",function(){ //corp has priority
...
function Steal()
{
	...
	OpportunityForAvoidPrevent(corp,"steal",function(){
		...
		TriggeredResponsePhase(runner,"stolen",function(){ //runner has priority
...
function TrashPrograms(num)
{
	...
	var programOptions = ChoicesInstalledCards(runner,function(card){ return CheckCardType(card,["program"]); });
	...
	DecisionPhase(corp,programOptions,decisionCallback,null,"Trash 1 program",this);
}
...
function Expose(card)
{
	...
	OpportunityForAvoidPrevent(otherPlayer,"expose",function(){
		...
		DecisionPhase(otherPlayer,[{}],decisionCallback,"Exposing "+GetTitle(intended.expose)).requireHumanInput = true; //finish viewing exposed card
	});
}
...
function Reveal(card,callback,context)
{
	...
	DecisionPhase(otherPlayer,[{}],decisionCallback,"Revealing "+GetTitle(card)).requireHumanInput = true; //finish viewing revealed card
}
...
function AddCounters(card,counter,num=1) {
	...
	card[counter]+=num;
	if (num==1) Log("1 "+counter+" counter placed on "+GetTitle(card,true));
...
function RemoveCounters(card,counter,num=1) {
	if (typeof(card[counter]) === 'undefined') card[counter] = 0;
	card[counter]-=num;
	if (num==1) Log("1 "+counter+" counter removed from "+GetTitle(card,true));
	else Log(num+" "+counter+" counters removed from "+GetTitle(card,true));
	UpdateCounters();
};
...
function Trace(baseStrength,callback,context)
{
	...
	Log("Trace initiated");
	var corpChoices = [{num:0, label:"Continue without increasing trace strength"}];
	var i=1;
	while (CheckCredits(i,corp,"trace"))
	{
		corpChoices.push({num:i, label:i+"[c]: Increase trace strength by "+i});
		i++;
	}
	function decisionCallbackA(paramsA) {
		SpendCredits(corp,paramsA.num,"trace",null,function(){
			traceStrength+=paramsA.num;
			if (paramsA.num > 0) Log("Trace strength increased to "+traceStrength);
			else Log("Trace strength not increased (remains at "+traceStrength+")");
			//runner link turn
			var runnerChoices = [{num:0, label:"Continue without increasing link strength"}];
			var i=1;
			while (CheckCredits(i,runner,"trace"))
			{
				runnerChoices.push({num:i, label:i+"[c]: Increase link strength by "+i});
				i++;
			}
			function decisionCallbackB(paramsB) {
				var linkStrength = Link() + paramsB.num;
				SpendCredits(runner,paramsB.num,"trace",null,function(){
					if (paramsB.num > 0) Log("Link strength increased to "+linkStrength);
					else Log("Link strength not increased (remains at "+linkStrength+")");
			...
			DecisionPhase(runner,runnerChoices,decisionCallbackB,"Trace<sup>"+baseStrength+"</sup>","Increase link strength (from "+Link()+")",this);
		},this);
	}
	DecisionPhase(corp,corpChoices,decisionCallbackA,"Trace<sup>"+baseStrength+"</sup>","Increase trace strength (from "+baseStrength+")",this);
}
