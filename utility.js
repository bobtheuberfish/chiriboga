...
function DeckBuild(
  ...
  if (identityCard.player == runner) {
	  ...
	  if (setIdentifiers.includes('su21')) agendaCards = agendaCards.concat([31041, 31051,31052, 31061,31062, 31071, 31072]);
	  ...
	  if (setIdentifiers.includes('su21')) otherCards = otherCards.concat([31047, 31048, 31049, 31053, 31054, 31058, 31059, 31063, 31064, 31068, 31069]);
	  cardsAdded = cardsAdded.concat(DeckBuildRandomly(
		identityCard,
		otherCards,
		destination,
